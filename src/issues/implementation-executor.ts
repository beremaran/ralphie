    ...
    execute: async (input) => {
        try {
            ...
            if (
                ...
                if (
                    actual.head.toLowerCase() !==
                    storedCheckpoint.sha.toLowerCase()
                ) {
                    throw new RalphieError({
            ...
            const hasChanges = await operations.hasStagedChanges(
                context.repositoryPath,
            );
            if (!hasChanges) {
                ...
                return resolution.output.status ===
            ...
            for (
                ...
                if (
                    !(await operations.hasStagedChanges(context.repositoryPath))
                ) {
                    await progress.emit({
                        ...issueProgress(input),
