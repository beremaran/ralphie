...
staging and signing steps succeed. Existing published release assets are
validated through the anonymous URL template
`https://github.com/beremaran/ralphie/releases/download/<tag>/<asset-name>`
and accepted only when their SHA-256 matches the locally generated bytes;
missing assets may be added, but differing assets fail closed and are never
replaced. An already-published handle is reconciled without republishing.
...
`RALPHIE_COMMIT_SHA`; GitHub tokens, OIDC tokens, and Codex/model credentials are
