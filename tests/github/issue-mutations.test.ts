...
describe("GitHub issue mutations", () => {
        ...
        const client = {
            rest: {
                issues: {
                    ...
                    update: async () =>
                        issueResponse(31, "Child issue", "Child body"),
                },
            },
        } as unknown as Octokit;

        const issue = await mutations.create(
            client,
            "https://github.com/owner/repository.git",
            { title: "Child issue", body: "Child body" },
        );
    ...
    test("updates title and body through Octokit", async () => {
        ...
        await mutations.update(client, "owner/repository", 32, {
            title: "Updated title",
            body: "Updated body",
        });
    ...
    test("treats an issue already closed for the requested reason as success", async () => {
        ...
        const issue = await mutations.close(
            client,
            "owner/repository",
            34,
            "completed",
    ...
    test("maps Octokit failures into RalphieError", async () => {
        ...
        ).rejects.toThrow("Failed to create an issue");
    });
});