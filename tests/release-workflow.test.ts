...
describe("release container metadata contract", () => {
    ...
    test("attaches SPDX SBOM and SLSA provenance to each container digest before aliases", async () => {
        const workflow = await readRepositoryFile(
            ".github/workflows/release.yml",
        );
        const pushJobStart = workflow.indexOf("  push-container:");
        ...
        const sbomActionPin =
            "anchore/sbom-action@006b7ce8314066bdf1765b4500370d40fa6917a3";
        const sbomAttestPin =
            "actions/attest-sbom@c604332985a26aa8cf1bdc465b92731239ec6b9e";
        const provenanceAttestPin =
            "actions/attest-build-provenance@bd77c077858b8d561b7a36cbe48ef4cc642ca39d";
        const attestationSubjectsStart = pushJob.indexOf(
            "name: Validate workflow identity and persist container attestation subjects",
        );
        const sbomValidationStart = pushJob.indexOf(
            "name: Validate and pin container SBOMs to release identity",
        );
        const verifyStart = pushJob.indexOf(
            "name: Verify all container attestations exist and match subjects",
        );
        const aliasStart = pushJob.indexOf(
            "name: Create manifest aliases from immutable digests",
        );
        const subjectsUploadStart = pushJob.indexOf(
            "name: Upload verified publication subjects",
        );

        // The exact minimum permission set for promotion plus attestations:
        // GHCR writes, attestation storage records, and Actions OIDC. No
        // unrelated write permission (in particular no contents: write) is
        // granted to the job.
        expect(pushJob).toContain("packages: write");
        expect(pushJob).toContain("attestations: write");
        expect(pushJob).toContain("id-token: write");
        expect(pushJob).not.toContain("contents: write");

        // The two-entry subject map is consumed after platform digest
        // verification and before any mutable alias, and the OIDC identity
        // is pinned to the validated protected tag and exact commit so an
        // attestation action can never infer a default branch or mutable
        // tag.
        expect(subjectsUploadStart).toBeGreaterThan(-1);
        expect(attestationSubjectsStart).toBeGreaterThan(subjectsUploadStart);
        expect(attestationSubjectsStart).toBeLessThan(aliasStart);
        expect(pushJob).toContain('test "$GITHUB_REF" = "refs/tags/$TAG"');
        expect(pushJob).toContain('test "$GITHUB_SHA" = "$SOURCE_REF"');
        expect(pushJob).toContain(
            'test "$GITHUB_WORKFLOW_REF" = "$GITHUB_REPOSITORY/.github/workflows/release.yml@refs/tags/$TAG"',
        );
        expect(pushJob).toContain("ralphie.container-attestation-subjects.v1");

        // One SPDX 2.3 SBOM per platform, generated by scanning the
        // immutable registry digest itself (never an OCI archive or a
        // mutable tag), with immutable action pins.
        expect(pushJob.match(new RegExp(sbomActionPin, "g"))).toHaveLength(2);
        expect(pushJob).toContain(
            "image: ghcr.io/beremaran/ralphie@${{ steps.attestation-subjects.outputs.amd64_digest }}",
        );
        expect(pushJob).toContain(
            "image: ghcr.io/beremaran/ralphie@${{ steps.attestation-subjects.outputs.arm64_digest }}",
        );
        expect(pushJob).toContain("format: spdx-json");
        for (const arch of ["amd64", "arm64"]) {
            expect(pushJob).toContain(
                `output-file: ralphie-container-${arch}.sbom.spdx.json`,
            );
        }

        // The SBOMs are bound to the validated release identity and
        // re-validated as SPDX 2.3 JSON by the checked-in seam before any
        // attestation runs.
        expect(pushJob).toContain("bun scripts/validate-container-sboms.ts");
        expect(pushJob).toContain(
            "--subjects container-attestation-subjects.json",
        );
        expect(pushJob).toContain("bun install --frozen-lockfile");
        expect(sbomValidationStart).toBeGreaterThan(attestationSubjectsStart);

        // Each digest receives both attestation kinds with the canonical
        // subject-name and the exact subject-digest from the persisted map,
        // registry/referrer push enabled, both platforms required, all
        // actions pinned to immutable SHAs.
        expect(pushJob.match(new RegExp(sbomAttestPin, "g"))).toHaveLength(2);
        expect(
            pushJob.match(new RegExp(provenanceAttestPin, "g")),
        ).toHaveLength(2);
        expect(pushJob).toContain(
            "subject-digest: ${{ steps.attestation-subjects.outputs.amd64_digest }}",
        );
        expect(pushJob).toContain(
            "subject-digest: ${{ steps.attestation-subjects.outputs.arm64_digest }}",
        );
        expect(
            pushJob.match(/subject-name: ghcr.io\/beremaran\/ralphie/g),
        ).toHaveLength(4);
        expect(
            pushJob.match(
                /sbom-path: ralphie-container-(?:amd64|arm64)\.sbom\.spdx\.json/g,
            ),
        ).toHaveLength(2);
        expect(pushJob.match(/push-to-registry: true/g)).toHaveLength(4);
        expect(pushJob).toContain(
            "--predicate-type https://slsa.dev/provenance/v1",
        );

        // Both attestation kinds are verified for both platforms before any
        // mutable alias. `gh attestation verify` runs on the exact immutable
        // `oci://ghcr.io/beremaran/ralphie@sha256:<digest>` reference (a
        // bare reference would be treated as a non-existent local file) for
        // the SLSA provenance v1 and SPDX Document predicates with the
        // validated workflow identity and commit, authenticated to GHCR
        // through an explicit $DOCKER_CONFIG keychain entry rather than the
        // login action's configuration file. The gates are presence-based:
        // bundle records are keyed by signature, so re-running an
        // interrupted attempt leaves extra records for the same digest, and
        // at least one verified attestation per predicate kind must match
        // the exact run identity, the exact annotated SBOM bytes, and the
        // recorded SBOM sha256 (never an exact record total).
        expect(verifyStart).toBeGreaterThan(-1);
        expect(verifyStart).toBeLessThan(aliasStart);
        expect(
            pushJob.match(/gh attestation verify "oci:\/\/\$reference"/g),
        ).toHaveLength(2);
        expect(pushJob).not.toContain('gh attestation verify "$reference"');
        expect(pushJob).toContain('export DOCKER_CONFIG="$docker_config"');
        expect(pushJob).toContain("base64 -w0");
        expect(pushJob).toContain("any(.[];");
        expect(pushJob).toContain('--slurpfile local_sbom "$sbom"');
        expect(pushJob).toContain("$predicate == $local_sbom[0]");
        expect(pushJob).toContain(
            'test "$(sha256sum "$sbom" | cut -d \' \' -f1)" = "$sbom_sha256"',
        );
        expect(pushJob).not.toContain("attestations/sha256:$digest");
        expect(pushJob).not.toContain("attestations[]] | length == 2");
        expect(pushJob).toContain(
        ...
        expect(pushJob).toContain('--source-ref "refs/tags/$TAG"');
        expect(pushJob).toContain(
            "--cert-oidc-issuer https://token.actions.githubusercontent.com",
        );
        expect(pushJob).toContain("resolvedDependencies");
        expect(pushJob).toContain("invocationId");
        expect(pushJob).toContain(".sbom_sha256 | test");

        // Every generator/attachment failure fails the job: all three new
        // shell steps run with set -euo pipefail and nothing in the job
        // uses continue-on-error, a swallowed error, or a fallback.
        const newShellSteps = [
            "Validate workflow identity and persist container attestation subjects",
            "Validate and pin container SBOMs to release identity",
            "Verify all container attestations exist and match subjects",
        ];
        for (const [index, name] of newShellSteps.entries()) {
            const start = pushJob.indexOf(`name: ${name}`);
            const end =
                index + 1 < newShellSteps.length
                    ? pushJob.indexOf(`name: ${newShellSteps[index + 1]}`)
                    : aliasStart;
            const step = pushJob.slice(start, end);
            expect(step).toContain("set -euo pipefail");
            expect(step).not.toContain("continue-on-error:");
        }
        expect(pushJob).not.toContain("continue-on-error:");
        expect(pushJob).not.toContain("continue-on-error: true");
        expect(pushJob).not.toContain("continue-on-error: false");

        // Aliases are created only after successful SBOM generation, SBOM
        // and provenance attachment, and verification of both platforms.
        expect(verifyStart).toBeLessThan(aliasStart);
        const firstSbomAttest = pushJob.indexOf(sbomAttestPin);
        const firstProvenanceAttest = pushJob.indexOf(provenanceAttestPin);
        expect(firstSbomAttest).toBeGreaterThan(sbomValidationStart);
        expect(firstSbomAttest).toBeGreaterThan(-1);
        expect(firstProvenanceAttest).toBeGreaterThan(firstSbomAttest);
        expect(aliasStart).toBeGreaterThan(firstProvenanceAttest);
    });
...
printf 'upload\\n'`,
    ...
    test("attests each exact final binary with least privilege and digest evidence", async () => {
        ...
        // `attestations: write` is confined to the two protected publishers:
        // this binary-attestation job (publish) and the container
        // attestation job (push-container). No earlier job holds it.
        expect(
            workflow.slice(0, workflow.indexOf("  push-container:")),
